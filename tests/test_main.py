import pytest
import os
from dcs_log_viewer.main import _lifespan, main
...
async def test_app_lifespan():
    ...
    mock_app = MagicMock()
    with patch("dcs_log_viewer.main.load_config") as mock_load:
        mock_load.return_value = {"theme": "custom"}
        with patch("dcs_log_viewer.main.ws_state.set_config") as mock_set:
...
def test_main_cli_defaults():
    ...
    with patch("dcs_log_viewer.main.uvicorn.run") as mock_run:
        with patch("dcs_log_viewer.main.os.environ", {}):
...
def test_main_cli_custom_env():
    ...
    with patch("dcs_log_viewer.main.uvicorn.run") as mock_run:
        with patch("dcs_log_viewer.main.os.environ", custom_env):
